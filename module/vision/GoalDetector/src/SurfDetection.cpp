...
namespace module {
    namespace vision {
    	...
    	SurfDetection::SurfDetection(std::shared_ptr<const message::vision::ClassifiedImage<message::vision::ObjectClass>> frame_2)
	   				  : frame_p(frame_2)
	  	{
		...
		void SurfDetection::findLandmarks(std::unique_ptr<std::vector<Ipoint>>& landmarks_out,std::unique_ptr<Eigen::VectorXf>& landmark_tf,std::unique_ptr<std::vector< std::vector<float>>>& landmark_pixLoc){
			...
			if(left_horizon >= 0 && left_horizon < h && right_horizon >= 0 && right_horizon < h){   /********** Need to check that the horizon is in the image ********************/
		...
		inline void SurfDetection::getHorizonDescriptor()
		{
			...
			std::vector<Ipoint>::iterator ipt = landmarks->begin()+index;    // replaces: Ipoint *ipt = &frame_p->landmarks[index];
			...
			for(int subregion = -1; subregion<2; subregion++){
				...
				for(int sample = 0; sample < SURF_DESCRIPTOR_SAMPLES; sample++){
		      		sample_x = fRound(x + (subregion*SURF_DESCRIPTOR_SAMPLES*scale + (sample-(SURF_DESCRIPTOR_SAMPLES/2))*scale) );
